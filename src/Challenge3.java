...
//		}
	...
	public void askUAVToSweep(OutputStream out, long id, double lat1, double lon1, double lat2, double lon2,
			double latinc) throws Exception {
		...
		/*
		 * double lat1 = 53.2951; double lat2 = 53.3395; double longt2 = -1.9093; double
		 * startlon = -2.0071;
		 */
...
/*
 * if(!checkValidPos(avs, id)) { if(!hasTurned.containsKey(id) ||
 * !hasTurned.get(id)) { reverseHeading(avs, id, out); hasTurned.put(id, true);
 * System.out.println("reversing"); } } else { if(hasTurned.containsKey(id)) {
 * hasTurned.remove(id); } }
 */

/*
 * public void reverseHeading(AirVehicleState avs, long id, OutputStream out)
 * throws IOException, Exception {
 * 
 * VehicleActionCommand vac = new VehicleActionCommand(); vac.setVehicleID(id);
 * vac.setStatus(CommandStatusType.Pending); vac.setCommandID(1);
 * 
 * int h; if(id == 1 || id == 2){ h = 90; } else { h = -90; }
 * FlightDirectorAction fda = new FlightDirectorAction();
 * fda.setHeading(avs.getHeading()+h); vac.getVehicleActionList().add(fda);
 * 
 * out.write(avtas.lmcp.LMCPFactory.packMessage(vac, true));
 * 
 * flags.get(id).set(1, flags.get(id).get(0)); flags.get(id).set(0, false); }
 */