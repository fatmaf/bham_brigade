 ...
 */
public class HDSClientHeatMap extends Thread {
	...
	public HDSClientHeatMap() {
		...
		heatMap.run(x -> {
			...
			double max_lat = (double)x.getMaxLat(); 
			double max_lon = (double)x.getMaxLong();
			double min_lat = (double)x.getMinLat(); 
			double min_lon = (double)x.getMinLat();
			numCellsSide = x.getSize(0); 
	...
	public Socket connect(String host, int port) {
		...
		try {
			...
			try {
				...
				Logger.getLogger(HDSClientHeatMap.class.getName()).log(Level.SEVERE, null, ex1);
			}
...
//			Point cell = locCell(rect.getCenterPoint());
	...
	public void run() {
		try {
			...
			Logger.getLogger(HDSClientHeatMap.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
	...
	public void setLimitsUsingRect(Rectangle bounds) throws Exception {
			...
			setLimitsUsingHighLow(high_loc.getLatitude(),high_loc.getLongitude(),low_loc.getLatitude(),low_loc.getLongitude());
