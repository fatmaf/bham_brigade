...
public class UnknownSearch extends Thread {
	 ...
	 * @throws Exception
	 */
	public void sendLoiterCommand(OutputStream out, InputStream in, long vehicleId, Location3D location)
			throws Exception {
...
// method for making 4 UAVs to make a cross formation  
	...
	public void makingCross(OutputStream out, int numberOfUAVsSearch, double latincrement, double longincrement)
			throws Exception {

		// Location3D location = new Location3D(53.4811,-1.8119, 0,
		// afrl.cmasi.AltitudeType.MSL);
		/*
...
//searching command parallel search
	 ...
	 * 
	 * 
	 * double lat1 = 53.4463; double lat2 = 53.4914; double longt2 = -1.8341; double
	 * startlon = -1.7584;
	 * 
	 * 
	 * 
	 * double diflat = lat2 - lat1; double diflongt = longt2 - startlon; double
	 * longtshare = diflongt/numberOfUAVsSearch; double latshare =
	 * diflat/numberOfUAVsSearch;
	 * 
	 * double lon =longtshare/2+circleadd; double longt1 = longtshare/2; lat1 =
	 * lat2/2; longt1 = longtshare/2;
	 * 
	 * double latincrement = 0.0057;
	 * 
	 * 
	 * 
	 * int ct = 0; int rd = 1; //Setting up the mission to send to the UAV
	 * 
	 * MissionCommand o = new MissionCommand(); o.setFirstWaypoint(1); //Setting the
	 * UAV to recieve the mission o.setVehicleID(3);
	 * o.setStatus(CommandStatusType.Pending); //Setting a unique mission command ID
	 * o.setCommandID(1);
	 * 
	 * //Creating the list of waypoints to be sent with the mission command
	 * ArrayList<Waypoint> waypoints = new ArrayList<Waypoint>(); //Creating the
	 * first waypoint //Note: all the following attributes must be set to avoid
	 * issues
	 * 
	 * while(lat1<=lat2){
	 * 
	 * Waypoint waypoint = new Waypoint(); //Setting 3D coordinates
	 * 
	 * if(ct == 0 ){ waypoint.setLatitude(lat1); waypoint.setLongitude(longt1); }
	 * else if(ct == 1){
	 * 
	 * waypoint.setLatitude(lat1); longt1 =longt1+longtshare;
	 * waypoint.setLongitude(longt1); } else if(ct % 2 == 0){
	 * lat1=lat1+latincrement; waypoint.setLatitude(lat1);
	 * waypoint.setLongitude(longt1);
	 * 
	 * }
	 * 
	 ...
	 * if(rd % 2==0) {
	 * 
	 * longt1 = longt1-longtshare; waypoint.setLatitude(lat1);
	 * waypoint.setLongitude(longt1); } else { longt1 = longt1+longtshare;
	 * waypoint.setLatitude(lat1); waypoint.setLongitude(longt1); }
	 * 
	 * 
	 * }
	 * 
	 * 
	 * waypoint.setAltitude(150); waypoint.setAltitudeType(AltitudeType.MSL);
	 * //Setting unique ID for the waypoint waypoint.setNumber(ct);
	 * waypoint.setNextWaypoint(ct+1); //Setting speed to reach the waypoint
	 * waypoint.setSpeed(30); waypoint.setSpeedType(SpeedType.Airspeed); //Setting
	 * the climb rate to reach new altitude (if applicable)
	 * waypoint.setClimbRate(0); waypoint.setTurnType(TurnType.TurnShort); //Setting
	 * backup waypoints if new waypoint can't be reached
	 * waypoint.setContingencyWaypointA(ct-1);
	 * waypoint.setContingencyWaypointB(ct-2);
	 * 
	 * waypoints.add(waypoint);
	 ...
	 * //Sending the Mission Command message to AMASE to be interpreted
	 * out.write(avtas.lmcp.LMCPFactory.packMessage(o, true));
	 * System.out.println("Mission sent to UAV "+id3 +
	 * "Diagonal istance is "+computeDistance(lat1,lat2,longt1,longt2)+" meters");
	 * circleadd = circleadd + longtshare; }
	 * 
	 * if(id4==4) {
	 * 
	 ...
	 * 
	 * MissionCommand o = new MissionCommand(); o.setFirstWaypoint(1); //Setting the
	 * UAV to recieve the mission o.setVehicleID(id4);
	 * o.setStatus(CommandStatusType.Pending); //Setting a unique mission command ID
	 * o.setCommandID(1);
	 * 
	 * //Creating the list of waypoints to be sent with the mission command
	 ...
	 * 
	 * 
	 * public void searchMissionExpand2(OutputStream out, int numberOfUAVsExpand)
	 * throws Exception {
	 * 
	 * double circleadd = 0.0; //int
	 * 
	 * 
	 * 
	 ...
	 * 
	 * double lat1 = 53.4463; double lat2 = 53.4914; double longt2 = -1.8341; double
	 * startlon = -1.7584; double lon =-1.7584+circleadd; double longt1 = lon;
	 * double diflat = lat2 - lat1; double diflongt = longt2 - startlon; double
	 * longtshare = diflongt/numberOfUAVsSearch; double latshare =
	 * diflat/numberOfUAVsSearch; double latincrement = 0.0057;
	 * 
	 * 
	 * 
	 * int ct = 0; int rd = 1; //Setting up the mission to send to the UAV
	 * 
	 * MissionCommand o = new MissionCommand(); o.setFirstWaypoint(1); //Setting the
	 * UAV to recieve the mission o.setVehicleID(n);
	 * o.setStatus(CommandStatusType.Pending); //Setting a unique mission command ID
	 * o.setCommandID(1);
	 * 
	 * //Creating the list of waypoints to be sent with the mission command
	 ...
	 * //Sending the Mission Command message to AMASE to be interpreted
	 * out.write(avtas.lmcp.LMCPFactory.packMessage(o, true));
	 * System.out.println("Mission sent to UAV "+n +
	 * "Diagonal istance is "+computeDistance(lat1,lat2,longt1,longt2)+" meters");
	 * circleadd = circleadd + longtshare; }
	 * 
	 */
	...
	public void readMessages(InputStream in, OutputStream out) throws Exception {
		...
		if (o instanceof afrl.cmasi.EntityState) {
			...
			if (energyAvail <= 25) {
				...
				/*
				 * double rate = (40/energyrate); System.out.println(energyAvail);
				 * if(energyAvail <= 25) { sendMissionCommand = false; Location3D c = new
				 * Location3D(clat, clongt, 0, afrl.cmasi.AltitudeType.MSL);
				 * 
				 * //goForCharge(out,((afrl.cmasi.EntityState) o).getID(), c);
				 * 
				 * sendKnownMission(out, c);
				 * 
				 * if(energyAvail == 100) { sendMissionCommand = true; }
				 * 
				 * 
				 * double rate2 = (25/energyrate);
				 * System.out.println("UAV3 or UAV4"+energyAvail); if(energyAvail <= 25) {
				 * sendMissionCommand = false; //Location3D c = new Location3D(clat, clongt, 0,
				 * afrl.cmasi.AltitudeType.MSL);
				 * 
				 * //goForCharge(out,((afrl.cmasi.EntityState) o).getID(), c);
				 * sendKnownMission(out, c); if(energyAvail == 100) { sendMissionCommand = true;
				 * }
				 * 
				 */
				...
				/*
				 * 
				 * System.out.println( "Energy Available in %" +energyAvail +"/n"+ "Pitch"
				 * +pitch +"/n"+ "Energy Rate" +energyrate +"/n"+ "Vehicle ID" +vehicle_ID
				 * +"/n"+ "Velocity x dir" +vx +"/n"+ "Velocity y dir" +vy +"/n"+
				 * "Velocity z dir" +vz +"/n"+ "Accelaration x dir" +ax +"/n"+
				 * "Accelaration y dir" +ay +"/n"+ "Accelaration z dir" +az +"/n"+
				 * "Location timer" +loc +"/n"+ "Time " +time +"/n"
				 * 
				 * );
				 */
			}
